...
public class ProductoImagenWebpPublicController : ControllerBase
{
    ...
    private readonly ProductoWebpTranscodeGate _transcodeGate;
    ...
    public ProductoImagenWebpPublicController(
        ...
        ProductoWebpTranscodeGate transcodeGate)
    {
    ...
    public async Task<IActionResult> GetProductoWebp(int id, CancellationToken cancellationToken)
    {
        ...
        await _transcodeGate.Semaphore.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            ...
            try
            {
                webp = await _webpEncoder
                    .EncodeToWebpAsync(capped, opt.WebpQuality, cancellationToken, opt.MaxProxyImageEdgePixels)
                    .ConfigureAwait(false);
            }
            catch (Exception ex)
            {
